 ...
 */
public class DDENonLinearApp {

  static public void main (String[] args) {

    // ODE
    ODE ode = new DDENonLinear();
    double initTime = 0, maxTime = 1.5;
  ...
  static private double solution(double time) {
    return Math.exp(time*time);
  }

  static class DDENonLinear implements DelayDifferentialEquation {
    static private double sAlpha = 0.4;

    private double[] mState = { solution(0.1), 0.1};
    ...
    public void getRate(double[] state, double[] rate){    
      rate[0] =  2 * Math.sqrt(Math.log(state[0])) * mHistory.interpolate(state[1]-sAlpha, 0) * Math.exp(2*sAlpha*Math.sqrt(Math.log(state[0]))) * Math.exp(-sAlpha*sAlpha);
      rate[1] = 1; // time
    }
    ...
    public double getMaximumDelay() { return sAlpha; }

    public double[] getDelays(double[] state) { return new double[] { sAlpha }; }

    public void getInitialCondition(double time, double state[]){
      state[0] = Math.exp(time*time);
      state[1] = time;
    }
